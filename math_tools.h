...
//FUNCIONAS PROPORSIONADAS POR EL INSTRCTOR 
...
// FUNCION MATRIZ INVERSA
void matrizInversa(Matrix Original, Matrix &Inversa){
  
  Matrix cofactor;
  Matrix transpuesta;

  zeroes(Inversa, cofactor.size());
  zeroes(cofactor, cofactor.size());
  zeroes(transpuesta, cofactor.size());
  
  float determinante =   determinant(Original);
  ...
  if(determinante == 0){
    ...
    productRealMatrix(x, cofactor, Inversa);
  }
